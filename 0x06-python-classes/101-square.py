...
class Square:
    ...
    def my_print(self):
        ...
        The position is used to add spaces before each line.
        """
        if self.size == 0:
            print()
        else:
    ...
    def __str__(self):
        ...
        for _ in range(self.size):
            square_rows.append(" " * self.position[0] + "#" * self.size)
        
        return "\n".join(square_rows)
        
    def __repr__(self):
        """
        Returns a string representation of the square for debugging purposes.

        Returns:
            str: A string representation of the square.
        """
        return f"Square(size={self.size}, position={self.position})"
